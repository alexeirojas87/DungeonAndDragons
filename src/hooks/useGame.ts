...
import type { Character, NarrativeEntry, Language, Archetype, Origin, UIState, DialogueState } from '../engine/types';
...
export function useGame() {
  ...
  const startGame = useCallback((name: string, archetype: Archetype, origin: Origin) => {
    const engine = getEngine();
    const character = createCharacter(name, archetype, origin);
    engine.initGame(character);
   ...
   * it against the same contract the authored chapter satisfies, so a success
   * here is immediately playable; a failure leaves the save untouched.
   */
  const advanceChapter = useCallback(async () => {
    ...
    if (!result.ok) {
      setChapterError({ message: result.error, issues: result.issues });
      setIsGeneratingChapter(false);
      return;
    }

    engine.appendChapter(result.chapter);
    setNarrative([...engine.getNarrative()]);
    setActiveDialogue(null);
    saveGame(engine.getState(), engine.getNarrative(), language);
    saveCheckpoint(engine.getState(), engine.getNarrative(), language);

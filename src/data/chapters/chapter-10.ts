...
const NODES: Record<string, StoryNode> = {
  ...
  c10_hand_trust: {
    ...
    text: 'The court vindicated the door, or reformed the pact — an institution becomes the witness. The routes inside the hand of trust are two pairs: the Court Restored pair on a vindicated verdict, split by whether the name was returned; the New Concord pair on a reformed verdict, split by whether the map was shared. The history behind you picks the route; the hand only opens the door.',
    textEs: 'La corte vindicó la puerta, o reformó el pacto — una institución se vuelve el testigo. Las rutas dentro de la mano de la confianza son dos pares: el par de la Corte Restaurada sobre un veredicto vindicado, dividido por si el nombre fue devuelto; el par del Nuevo Concordato sobre un veredicto reformado, dividido por si el mapa fue compartido. La historia detrás de ti elige la ruta; la mano solo abre la puerta.',
    choices: [
  ...
  c10_hand_break: {
    ...
    text: 'The court dissolved the pact — no vessel of privilege; the terms end. The Decentralized pair comes first: the ash seed with a wall held and free witnesses, or the opened Vault with freed names. Then the Unbound pair: the map burned with a stranded Vault, or the registry free with a broken wall. The history behind you picks the route; the hand only opens the door.',
  ...
  c10_end_unbound_world: {
    ...
    textEs: 'Sin testigo y sin vasija. Cada promesa se paga una vez, toda a la vez, en una noche de austeridad. La décima puerta se cierra sobre un mundo sin ataduras, donde la suma que la campaña llevó se salda en un solo saldar y nada se lleva hacia adelante. El precio es la noche misma, y la mañana después es un mundo que no debe nada y no guarda nada. La campaña termina en un clean slate.',
  ...
  c10_end_decentralized_oaths: {
    ...
    text: 'Every community becomes its own witness, and each community own small vessel — a well, a bell, a ledger — becomes the vessel. Each community shoulders its own fraction forever. The Tenth Door closes on a world where no single keeper holds the whole, and the price is the fraction every community carries for as long as it stands. The campaign ends on a world of small vessels.',
